#!/usr/bin/env python3
"""
Compute Reserve Margin from DAM Ancillary Service Plan and Actual System Load - FIXED
======================================================================================

CRITICAL BUG FIX:
Both DAM AS Plan and Actual System Load parquets have datetime_local set to MIDNIGHT
for all 24 hours of each day. The actual hour is in the HourEnding column.

We must reconstruct proper hourly timestamps from DeliveryDate/OperDay + HourEnding!
...
print("="*80)
print("COMPUTING RESERVE MARGIN FROM DAM AS PLAN AND ACTUAL LOAD - FIXED VERSION")
print("="*80)
...
# ============================================================================
# 1. LOAD DAM ANCILLARY SERVICE PLAN AND FIX TIMESTAMPS
# ============================================================================
print("\n" + "="*80)
print("1. LOADING DAM ANCILLARY SERVICE PLAN AND FIXING TIMESTAMPS")
print("="*80)
...
for file in as_files:
    ...
    # BUG FIX: Reconstruct proper hourly timestamp from DeliveryDate + HourEnding
    df_fixed = df.with_columns([
        # Extract hour number from HourEnding
        pl.col('HourEnding').str.split(':').list.get(0).cast(pl.Int32).alias('hour_num'),
    ]).with_columns([
        # Construct proper hourly timestamp
        (pl.col('DeliveryDate').dt.replace_time_zone(None) +
         pl.duration(hours=pl.col('hour_num') - 1)).alias('timestamp')
    ])

    all_as.append(df_fixed)

    unique_hours = df_fixed.select('timestamp').unique().height
    print(f"  ✓ {year}: {len(df_fixed):,} records, {unique_hours:,} unique hours")

df_as = pl.concat(all_as)
print(f"\nTotal AS records: {len(df_as):,}")
print(f"Unique hourly timestamps: {df_as.select('timestamp').unique().height:,}")
...
print(f"✓ Pivoted to {len(df_as_pivot):,} hourly timestamps")
...
# ============================================================================
# 2. LOAD ACTUAL SYSTEM LOAD AND FIX TIMESTAMPS
# ============================================================================
print("\n" + "="*80)
print("2. LOADING ACTUAL SYSTEM LOAD AND FIXING TIMESTAMPS")
print("="*80)
...
for file in load_files:
    ...
    # BUG FIX: Reconstruct proper hourly timestamp from OperDay + HourEnding
    df_fixed = df.with_columns([
        # Extract hour number from HourEnding
        pl.col('HourEnding').str.split(':').list.get(0).cast(pl.Int32).alias('hour_num'),
    ]).with_columns([
        # Construct proper hourly timestamp
        (pl.col('OperDay').dt.replace_time_zone(None) +
         pl.duration(hours=pl.col('hour_num') - 1)).alias('timestamp')
    ])

    all_load.append(df_fixed)
...
print(f"Unique hourly timestamps: {df_load.select('timestamp').unique().height:,}")
...
stats = df_load_clean.select('actual_system_load_MW').describe()
mean_load = stats.filter(pl.col('statistic') == 'mean')['actual_system_load_MW'][0]
min_load = stats.filter(pl.col('statistic') == 'min')['actual_system_load_MW'][0]
max_load = stats.filter(pl.col('statistic') == 'max')['actual_system_load_MW'][0]
...
# Check coverage
non_null_reserves = df_merged.filter(pl.col('total_dam_reserves_MW').is_not_null()).height
total = len(df_merged)
print(f"Records with reserves data: {non_null_reserves:,} / {total:,} ({100*non_null_reserves/total:.1f}%)")
...
print(f"  Unique hourly timestamps: {df_merged.select('timestamp').unique().height:,}")
