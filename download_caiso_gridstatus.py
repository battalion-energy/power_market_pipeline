#!/usr/bin/env python3
"""
CAISO data downloader using gridstatus library with auto-resume.
...
Usage:
    ...
    python download_caiso_gridstatus.py --start-date 2019-01-01

    # Auto-resume (starts from last downloaded date or 2019-01-01)
    python download_caiso_gridstatus.py --auto-resume

    # Test with small range
    python download_caiso_gridstatus.py --start-date 2024-01-01 --end-date 2024-01-03
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('caiso_gridstatus_download.log'),
...
class CAISOGridstatusDownloader:
    """CAISO downloader using gridstatus with resume capability."""

    def __init__(self, output_dir: Path):
        self.output_dir = output_dir
        self.csv_dir = output_dir / "CAISO_data" / "csv_files"
    ...
    def download_lmp_day(self, date: datetime, market: str):
        ...
        try:
            ...
            df = self.caiso.get_lmp(date=date, market=market, locations="ALL")
    ...
    def download_load_day(self, date: datetime):
        ...
        try:
            ...
            df = self.caiso.get_load(start=date, end=end_date)
    ...
    def download_fuel_mix_day(self, date: datetime):
        ...
        try:
            ...
            end_date = date + timedelta(days=1)
            df = self.caiso.get_fuel_mix(start=date, end=end_date)
    ...
    def download_date_range(self, start_date: datetime, end_date: datetime):
        ...
        logger.info(f"\n{'='*80}")
        logger.info(f"CAISO DATA DOWNLOAD (gridstatus)")
...
def main():
    parser = argparse.ArgumentParser(
        description="CAISO downloader using gridstatus with auto-resume"
    )
    parser.add_argument(
        "--start-date",
        type=str,
    ...
    # Create downloader
    downloader = CAISOGridstatusDownloader(output_dir)
