...
impl ErcotPriceProcessor {
    ...
    pub fn process_realtime_prices(&self) -> Result<()> {
        ...
        files_by_year.par_iter().for_each(|(year, year_files)| {
            ...
            println!("\nProcessing year {} with {} files", year, year_files.len());
    ...
    fn process_rt_year(&self, year: i32, files: &[PathBuf], output_dir: &Path) -> Result<()> {
        ...
        // Debug first few dataframes
        for (i, df) in all_data.iter().take(3).enumerate() {
            println!("DataFrame {} columns: {:?}", i, df.get_column_names());
            println!("DataFrame {} shape: {:?}", i, df.shape());
        }

        // Define all expected columns (DSTFlag is optional)
        let required_columns = vec!["datetime", "DeliveryDate", "DeliveryHour", "DeliveryInterval", 
                                    "SettlementPointName", "SettlementPointType", "SettlementPointPrice"];
        
        // Normalize all dataframes to have the same columns
        let normalized_data: Result<Vec<DataFrame>> = all_data.into_iter()
            .map(|mut df| -> Result<DataFrame> {
                // Add missing columns with null values
                for col_name in &required_columns {
                    if !df.get_column_names().contains(col_name) {
                        // Add null column of appropriate type
                        let null_series = match *col_name {
                            "datetime" => Series::new_null(col_name, df.height()),
                            "DeliveryHour" | "DeliveryInterval" => {
                                Series::full_null(col_name, df.height(), &DataType::UInt32)
                            },
        ...
        let lazy_frames: Vec<LazyFrame> = normalized_data.into_iter()
            .map(|df| df.lazy())
            .collect();
    ...
    fn read_rt_file(&self, file: &Path) -> Result<DataFrame> {
        // Specify dtypes to ensure price is read as Float64
        let mut schema_overwrite = Schema::new();
        schema_overwrite.with_column("SettlementPointPrice".into(), DataType::Float64);
        
        let df = CsvReader::from_path(file)?
            .has_header(true)
            .with_dtypes(Some(Arc::new(schema_overwrite)))
            .finish()?;
        ...
        for i in 0..df.height() {
            if let Some(date_str) = date_str_ca.get(i) {
                if let Ok(date) = NaiveDate::parse_from_str(date_str, "%m/%d/%Y") {
                    ...
                    if let Some(time) = NaiveTime::from_hms_opt(h - 1, minutes, 0) {
    ...
    fn process_da_year(&self, year: i32, files: &[PathBuf], output_dir: &Path) -> Result<()> {
        ...
        // Define all expected columns (DSTFlag is optional)
        let required_columns = vec!["datetime", "DeliveryDate", "HourEnding", 
        ...
        for i in 0..df.height() {
            if let (Some(date_str), Some(hour_str)) = (
                ...
                if let Ok(date) = NaiveDate::parse_from_str(date_str, "%m/%d/%Y") {
                    ...
                    if let Some(time) = NaiveTime::from_hms_opt(hour - 1, 0, 0) {
                        let dt = NaiveDateTime::new(date, time);
                        datetimes.push(Some(dt.and_utc().timestamp_millis()));
                    } else {
                        datetimes.push(None);
    ...
    fn read_as_file(&self, file: &Path) -> Result<DataFrame> {
        // Specify dtypes to ensure MCPC is read as Float64
        let mut schema_overwrite = Schema::new();
        schema_overwrite.with_column("MCPC".into(), DataType::Float64);
    ...
    fn group_files_by_year(&self, files: &[PathBuf]) -> Result<HashMap<i32, Vec<PathBuf>>> {
                ...
                for part in parts {
                    if part.len() == 8 && part.chars().all(|c| c.is_numeric()) {
                        if let Ok(year) = part[0..4].parse::<i32>() {
                            if (2010..=2030).contains(&year) { // Reasonable year range
                                files_by_year.entry(year).or_default().push(file.clone());
                                found_year = true;
                                break;
                            }
                        }
        ...
        if unmatched > 0 {
            eprintln!("Warning: {} files could not be grouped by year", unmatched);
        }
    ...
    fn create_rt_schema(&self, output_dir: &Path) -> Result<()> {
        let schema = json!({
            ...
            "columns": {
                ...
                "DSTFlag": {
                    "type": "Utf8",
                    "description": "Daylight Saving Time flag (Y/N) - may be absent in older data",
                    "optional": true
                }
            }
        });
