#!/usr/bin/env python3
"""
SPP data downloader using gridstatus library with auto-resume.
...
Usage:
    ...
    python download_spp_gridstatus.py --start-date 2019-01-01

    # Auto-resume (starts from last downloaded date or 2019-01-01)
    python download_spp_gridstatus.py --auto-resume

    # Test with small range
    python download_spp_gridstatus.py --start-date 2024-01-01 --end-date 2024-01-03
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('spp_gridstatus_download.log'),
...
class SPPGridstatusDownloader:
    """SPP downloader using gridstatus with resume capability."""

    def __init__(self, output_dir: Path):
        self.output_dir = output_dir
        self.csv_dir = output_dir / "SPP_data" / "csv_files"
    ...
    def download_lmp_da_day(self, date: datetime):
        ...
        try:
            ...
            df = self.spp.get_lmp_day_ahead_hourly(date=date, location_type="ALL")
    ...
    def download_lmp_rt_day(self, date: datetime):
        ...
        try:
            logger.info(f"Downloading {data_type} for {date.date()}...")
            df = self.spp.get_lmp_real_time_5_min_by_location(date=date, location_type="ALL")
    ...
    def download_load_day(self, date: datetime):
        ...
        try:
            ...
            df = self.spp.get_load(date=date)
    ...
    def download_fuel_mix_day(self, date: datetime):
        ...
        try:
            ...
            df = self.spp.get_fuel_mix(date=date)
    ...
    def download_date_range(self, start_date: datetime, end_date: datetime):
        ...
        logger.info(f"\n{'='*80}")
        logger.info(f"SPP DATA DOWNLOAD (gridstatus)")
...
def main():
    parser = argparse.ArgumentParser(
        description="SPP downloader using gridstatus with auto-resume"
    )
    parser.add_argument(
        "--start-date",
        type=str,
    ...
    # Create downloader
    downloader = SPPGridstatusDownloader(output_dir)
