#!/usr/bin/env python3
"""
IESO (Ontario) data downloader using gridstatus library with auto-resume.
...
Usage:
    ...
    python download_ieso_gridstatus.py --start-date 2019-01-01

    # Auto-resume (starts from last downloaded date or 2019-01-01)
    python download_ieso_gridstatus.py --auto-resume

    # Test with small range
    python download_ieso_gridstatus.py --start-date 2024-01-01 --end-date 2024-01-03
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('ieso_gridstatus_download.log'),
...
class IESOGridstatusDownloader:
    """IESO downloader using gridstatus with resume capability."""

    def __init__(self, output_dir: Path):
        self.output_dir = output_dir
        self.csv_dir = output_dir / "IESO_data" / "csv_files"
        self.csv_dir.mkdir(parents=True, exist_ok=True)
        self.ieso = gridstatus.IESO()
    ...
    def download_lmp_da_day(self, date: datetime):
        ...
        try:
            ...
            df = self.ieso.get_lmp_day_ahead_hourly(date=date)
    ...
    def download_lmp_rt_day(self, date: datetime):
        ...
        try:
            logger.info(f"Downloading {data_type} for {date.date()}...")
            df = self.ieso.get_lmp_real_time_5_min(date=date)
    ...
    def download_load_day(self, date: datetime):
        ...
        try:
            ...
            df = self.ieso.get_load(start=date, end=end_date)
    ...
    def download_fuel_mix_day(self, date: datetime):
        ...
        try:
            ...
            end_date = date + timedelta(days=1)
            df = self.ieso.get_fuel_mix(start=date, end=end_date)
    ...
    def download_date_range(self, start_date: datetime, end_date: datetime):
        ...
        logger.info(f"\n{'='*80}")
        logger.info(f"IESO DATA DOWNLOAD (gridstatus)")
        ...
        while current_date <= end_date:
            ...
            self.download_lmp_da_day(current_date)
            self.download_lmp_rt_day(current_date)
    ...
    def auto_resume(self, fallback_start: datetime, end_date: datetime):
        ...
        data_types = ["lmp_day_ahead_hourly", "lmp_real_time_5_min", "load", "fuel_mix"]
...
def main():
    parser = argparse.ArgumentParser(
        description="IESO downloader using gridstatus with auto-resume"
    )
    parser.add_argument(
        "--start-date",
        type=str,
    ...
    # Create downloader
    downloader = IESOGridstatusDownloader(output_dir)
