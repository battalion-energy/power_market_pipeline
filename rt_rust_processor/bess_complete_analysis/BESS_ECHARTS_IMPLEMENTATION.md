...
```typescript
// TBX vs Actual Performance Comparison
const tbxComparisonOption: echarts.EChartsOption = {
  title: {
    text: 'BESS Performance vs TB2 Potential',
    subtext: 'Capture Rate Analysis',
    left: 'center'
  },
  tooltip: {
    trigger: 'axis',
    ...
    formatter: (params: any) => {
      ...
      const tb2 = params[1];
      const rate = (actual.value / tb2.value * 100).toFixed(1);
      return `
        ${actual.name}<br/>
        Actual: $${(actual.value / 1000).toFixed(0)}k<br/>
        TB2: $${(tb2.value / 1000).toFixed(0)}k<br/>
        Capture Rate: ${rate}%
      `;
    }
  },
  legend: {
    data: ['Actual Revenue', 'TB2 Potential'],
    bottom: 0
  },
  grid: {
    left: '3%',
...
## 12. Geographic Heat Map (Texas)

```typescript
// Texas Map with TB2 Revenue Potential
const texasMapOption: echarts.EChartsOption = {
  title: {
    text: 'TB2 Revenue Potential Across Texas',
    subtext: 'Click to view time series',
    left: 'center'
  },
  tooltip: {
    trigger: 'item',
    formatter: (params: any) => {
      return `
        ${params.name}<br/>
        TB2 Potential: $${(params.value / 1000).toFixed(0)}k/MW-yr<br/>
        BESS Count: ${params.data.bessCount}<br/>
        Avg Capture: ${(params.data.avgCapture * 100).toFixed(1)}%
      `;
    }
  },
  visualMap: {
  ...
  series: [
    ...
    },
    {
      ...
      symbolSize: (val: any[]) => {
        return Math.sqrt(val[2]) * 3; // Size by capacity
      },
...
// Register Texas map (GeoJSON required)
echarts.registerMap('texas', texasGeoJSON);
```

## 13. TBX Capture Rate Gauge Dashboard

```typescript
// Multiple gauge dashboard for capture rates
const captureRateDashboard: echarts.EChartsOption = {
  title: {
    text: 'BESS TBX Capture Rates',
    left: 'center'
  },
  grid: [
    { left: '5%', top: '10%', width: '40%', height: '35%' },
    { right: '5%', top: '10%', width: '40%', height: '35%' },
    { left: '5%', bottom: '10%', width: '40%', height: '35%' },
    { right: '5%', bottom: '10%', width: '40%', height: '35%' }
  ],
  series: [
    // Top Left - Overall Fleet
    {
      type: 'gauge',
      center: ['25%', '30%'],
      radius: '30%',
    ...
    // Additional gauges for different categories...
  ]
};
```
